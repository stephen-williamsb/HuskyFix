# app/src/pages/32_Maintenance_JobDetail.py
...
# Status update form
st.markdown("---")
st.subheader("Update Status / ETA / Notify Tenant")
...
notify_message = st.text_input("Optional tenant message")

if st.button("Update Status & (optionally) Notify"):
    ...
    if res is not None:
        ...
        # send notification if message provided — route may not exist yet; see comment
        if notify_message:
            try:
                # NOTE: confirm your backend route. Sample expected endpoint:
                # POST /users/notifications
                # body: { "user_id": <student_id>, "message": "text", "requestID": 123 }
                notif_resp = requests.post(f"{API_BASE}/users/notifications", json={
                    "user_id": detail.get("studentRequestingID"),
                    "message": notify_message,
                    "requestID": request_id
                }, timeout=6)
                # If your backend doesn't have /users/notifications yet, implement endpoint that creates an in-app notification.
                notif_resp.raise_for_status()
                st.success("Tenant notified")
            except Exception as e:
                st.warning(f"Notification POST failed (route may be missing): {e}")
